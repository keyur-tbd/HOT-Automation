...
class BlinkitHOTScheduler:
    ...
    def _ensure_numeric_columns_as_strings(self, df: pd.DataFrame) -> pd.DataFrame:
        """Ensure ONLY Item_Code and po_number columns are converted to strings"""
        try:
            item_code_col = self.excel_config['item_code_column']
            po_number_col = self.excel_config['po_number_column']
            
            # Convert ONLY Item_Code column to string if it exists
            if item_code_col in df.columns:
                # Store original values
                original_type = df[item_code_col].dtype
                # Convert to string
                ...
                df[item_code_col] = df[item_code_col].replace('nan', '')
                self.log(f"Converted {item_code_col} from {original_type} to string", "INFO")
            
            # Convert ONLY po_number column to string if it exists
            if po_number_col in df.columns:
                # Store original values
                original_type = df[po_number_col].dtype
                # Convert to string
                df[po_number_col] = df[po_number_col].astype(str)
                # Remove .0 from integer values converted to float
                df[po_number_col] = df[po_number_col].str.replace(r'\.0$', '', regex=True)
                # Replace 'nan' string with empty string
                df[po_number_col] = df[po_number_col].replace('nan', '')
                self.log(f"Converted {po_number_col} from {original_type} to string", "INFO")
            
            return df
        except Exception as e:
            self.log(f"Error converting specific columns to strings: {str(e)}", "WARNING")
            return df
    ...
    def _append_to_sheet_with_source(self, spreadsheet_id: str, sheet_name: str, df: pd.DataFrame, 
        ...
        """Append DataFrame to Google Sheet - ONLY Item_Code and po_number as strings, rest preserved as they are"""
        try:
            ...
            # Identify which columns should be treated as strings (ONLY item_code and po_number)
            string_columns = {
                self.excel_config['item_code_column'],
                self.excel_config['po_number_column']
            }
            
            # Prepare values - convert to list but preserve types for non-string columns
            if include_headers:
            ...
            for idx, row in df.iterrows():
                ...
                for col in df.columns:
                    ...
                    if pd.isna(val):
                        row_values.append('')
                    # Force string ONLY for item_code and po_number columns
                    elif col in string_columns:
                        row_values.append(str(val))
                    # Preserve original type for ALL other columns (including source_file)
                    else:
            ...
            if not values or (include_headers and len(values) == 1):
            ...
            # Use USER_ENTERED to preserve number formats for non-string columns
            # but Item_Code and po_number will be stored as strings
            ...
            self.log(f"Appended {len(values)} rows to Google Sheet (Item_Code & po_number as strings, rest preserved as they are)", "INFO")
            
            # Add apostrophe prefix for ONLY Item_Code and po_number columns to force string display
            self._format_columns_as_text(spreadsheet_id, sheet_name, string_columns, include_headers, len(values))
            
        except Exception as e:
            self.log(f"Failed to append to Google Sheet: {str(e)}", "ERROR")
    
    def _format_columns_as_text(self, spreadsheet_id: str, sheet_name: str, string_columns: set, include_headers: bool, num_rows: int):
        """Add apostrophe prefix to ONLY item_code and po_number columns to force text format in Google Sheets"""
        try:
            # Get sheet ID
            ...
            all_data = self.sheets_service.spreadsheets().values().get(
                spreadsheetId=spreadsheet_id,
                range=f"{sheet_name}!A:A"
            ).execute()
            last_row = len(all_data.get('values', []))
            
            # Calculate start row (skip header if it was included)
            start_row = last_row - num_rows + (1 if include_headers else 0) + 1
            
            # Apply text format to specific columns using API
            requests = []
            ...
            if requests:
                ...
                self.log(f"Formatted {len(column_indices)} columns as text", "INFO")
            
        except Exception as e:
            self.log(f"Failed to format columns as text: {str(e)}", "WARNING")
